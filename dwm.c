...
{
    int x, w, tw = 0;
    int boxs = drw->fonts->h / 9; // box showing what tags have active windows
...
focusstack(const Arg *arg)
{
    ...
    for(p = NULL, c = selmon->cl->clients; c && (i || !ISVISIBLE(c, c->mon));
        i -= ISVISIBLE(c, c->mon) ? 1 : 0, p = c, c = c->next);
    focus(c ? c : p);
    restack(selmon);
}
...
setup(void)
{
    ...
    lrpad = drw->fonts->h;
    bh = TEXTW(tags[0]); // Make bar same height as tags width (makes square tags)
    updategeom();
    /* init atoms */
